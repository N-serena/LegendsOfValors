...
public class MarketController {
    ...
    private void buyLoop(Hero hero, Market market) {
        ...
        for (int i = 0; i < wares.size(); i++) {
            ...
            if (it.getMinLevel() >= 8) color = Colors.PURPLE;
            else if (it.getMinLevel() >= 4) color = Colors.CYAN;

            // PRINT WITH LEVEL CLEARLY VISIBLE
            System.out.printf(" %-3d %s%-20s%s %-8.0f %-5d\n",
                    (i+1),
                    color, it.getName(), Colors.RESET,
                    it.getPrice(),
        ...
        if (scanner.hasNextInt()) {
            ...
            if (id > 0 && id <= wares.size()) {
                ...
                if (hero.getLevel() < item.getMinLevel()) {
                    System.out.println(Colors.RED + "Cannot Buy: You need Level " + item.getMinLevel() + "!" + Colors.RESET);
                } else if (hero.getGold() < item.getPrice()) {
                    System.out.println(Colors.RED + "Cannot Buy: Not enough Gold!" + Colors.RESET);
                } else {
